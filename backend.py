...
#import util
class DigitClassificationNBDataset(Dataset):

    def __init__(self, model):
        self.conditionalProb = None
        self.prior = None
        self.DIGIT_DATUM_WIDTH = 28
        self.DIGIT_DATUM_HEIGHT = 28
        self.numTraining = 5000
        self.numTest = self.numValidation = 1000
        mnist_path = get_data_path("mnist.npz")

        with np.load(mnist_path) as data:
            train_images = data["train_images"][:self.numTraining]
            train_labels = data["train_labels"][:self.numTraining]
            test_images = data["test_images"][:self.numTest+self.numValidation]
            test_labels = data["test_labels"][:self.numTest+self.numValidation]

            assert len(train_images) == self.numTraining
            assert len(test_images) == self.numTest+self.numValidation
            val_images = test_images[0::2]
            val_labels = test_labels[0::2]
            test_images = test_images[1::2]
            test_labels = test_labels[1::2]

            self.trainingLabels = list(train_labels)
            self.testLabels = list(test_labels)
            self.validationLabels = list(val_labels)

            #shaping data in dictionary format
            width = self.DIGIT_DATUM_WIDTH
            height = self.DIGIT_DATUM_HEIGHT
            rawTrainingData = train_images.reshape(self.numTraining, width, height)
            rawTestData = test_images.reshape(self.numTest, width, height)
            rawValidationData = val_images.reshape(self.numValidation, width, height)

            self.trainingData = []
            for row in rawTrainingData:
                aimage = {(i, j):0 if row[i, j]==0 else 1 for i in range(width) for j in range(height)}
                self.trainingData.append(aimage)
            self.testData = []
            for row in rawTestData:
                aimage = {(i, j):0 if row[i, j]==0 else 1 for i in range(width) for j in range(height)}
                self.testData.append(aimage)
            self.validationData = []
            for row in rawValidationData:
                aimage = {(i, j):0 if row[i, j]==0 else 1 for i in range(width) for j in range(height)}
                self.validationData.append(aimage)

        if model.automaticTuning:
            self.kgrid = [0.0005, 0.001, 0.005, 0.01, 0.02, 0.05, 0.5, 1, 5, 10, 50]
        else:
            self.kgrid = [1]

        self.model = model

# create dataset for a Perceptron with decision line x + 2y = 1.
...
class RegressionDataset(Dataset):
    def __init__(self, model):
        x = np.expand_dims(np.linspace(-np.pi, np.pi, num=200), axis=1)
