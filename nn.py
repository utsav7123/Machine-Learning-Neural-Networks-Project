...
class SoftmaxLoss(FunctionNode):
    ...
    #Computes the log-probabilities of each class using log_softmax.
    #Calculates the mean cross-entropy loss by comparing the predicted log-probabilities to the true labels.
    @staticmethod
    def _forward(*inputs):
        assert len(inputs) == 2, "Expected 2 inputs, got {}".format(len(inputs))
        assert inputs[0].ndim == 2, (
    ...
    #Computes the gradient of the loss with respect to the logits (inputs[0]) and labels (inputs[1]).
    @staticmethod
    def _backward(gradient, *inputs):
        assert np.asarray(gradient).ndim == 0
        log_probs = SoftmaxLoss.log_softmax(inputs[0])
...
def gradients(loss, parameters):
    ...
    parameters.
    1-Traverses the computation graph, starting from the loss node, to identify all
    2-nodes involved in the computation (e.g., operations, inputs, parameters).
    3-Builds a reverse ordering (stored in tape) for efficient backpropagation.
    4-Iterates over the computation graph in reverse order (from loss to inputs).
    5-Calls _backward for each node to compute gradients w.r.t. its parents.
    6-Collects the gradients for the requested parameters and returns them as Constant objects.

    Accumulates the gradients for each parent node.
