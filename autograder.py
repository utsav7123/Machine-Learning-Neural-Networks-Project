...
class Tracker(object):
    ...
    def finalize(self):
        ...
        print("""
The grade above is only for information. Grading is done by TA's when running your code on their test set.
""")
...
@test('q1', points=6)
def check_digit_classification_naiveBayes(tracker):
    import models
    model = models.NaiveBayesDigitClassificationModel()
    dataset = backend.DigitClassificationNBDataset(model)

    tracker.add_points(2)  # Partial credit for passing sanity checks on loading data

    # Conduct training and testing
    print("Training...")
    model.train(dataset)
    print("Validating...")
    guesses = model.classify(dataset.validationData)
    correct = [guesses[i] == dataset.validationLabels[i] for i in range(len(dataset.validationLabels))].count(True)
    validation_accuracy = 100.0 * correct / len(dataset.validationLabels)
    print(str(correct), ("correct out of " + str(len(dataset.validationLabels)) + " (%.1f%%).") % validation_accuracy)
    print("Testing...")
    guesses = model.classify(dataset.testData)
    correct = [guesses[i] == dataset.testLabels[i] for i in range(len(dataset.testLabels))].count(True)
    test_accuracy = (100.0 * correct / len(dataset.testLabels))
    print(str(correct), ("correct out of " + str(len(dataset.testLabels)) + " (%.1f%%).") % test_accuracy)

    accuracy_threshold = 0.79
    if test_accuracy >= accuracy_threshold:
        print("Your final test set accuracy is: (%.1f%%)" % test_accuracy)
...
def check_perceptron(tracker):
    ...
    print("Sanity checking is complete. Now training perceptron")
...
def check_regression(tracker):
    ...
    if train_loss <= loss_threshold:
        print("Your final loss is: {:f} in {} number of processing iterations".format(train_loss, dataset.processed))
        tracker.add_points(4)
    else:
